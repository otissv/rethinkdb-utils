  ...
  return _ramda2.default.curry(function (tableExists) {
    return promise(function (resolve, reject) {
      ...
      return db.table(tableName).indexList().run().then(function (response) {
        var result = [].concat(_toConsumableArray(indexes)).reduce(function (prev, curr) {
...
function seed(_ref16) {
  ...
  return promise(function (resolve, reject) {
    ...
    // _checkDBExists ({ db, dbName }),
    // _createDB ({ db, dbName }),
    _checkTableExists({ db: db, tableName: tableName }), _createTable({ db: db, tableName: tableName }), _indexesDontExist({ db: db, tableName: tableName, indexes: indexes }), _createIndexes({ db: db, tableName: tableName, indexes: indexes }), _insertIntoTable({ db: db, tableName: tableName, data: data }), fn(resolve))();
  });
}